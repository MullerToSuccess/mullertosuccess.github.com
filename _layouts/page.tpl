...
<link rel="icon" href="/img/github.ico" type="image/x-icon">
...
<aside>
	...
	<nav class="block">
		<ul>
		{% for category in site.custom.categories %}<li class="{{ category.name }}"><a href="/技术向/{{ category.name }}/">{{ category.title }}</a></li>
		{% endfor %}
		</ul>
	</nav>
